...
type LeaveSectionProps = {
  ...
  actions: {
    ...
    onBulkWorkProcedureDecision: (decision: "approve" | "return") => Promise<void>;
    ...
    <section className="stack-section section-enter delay-3">
            ...
            { key: "status", header: "状態", render: (row) => props.formatters.formatApprovalStatus(row.status) },
            {
              ...
              render: (row) => (
                <div className="button-row">
                  ...
                  <button type="button" className="table-action" onClick={() => void props.actions.onWorkProcedureDecision(row.id, "approve")}>
                    承認
                  </button>
                  <button type="button" className="table-action" onClick={() => void props.actions.onWorkProcedureDecision(row.id, "return")}>
                    差戻し
                  </button>
                </div>
              ),
        ...
        <section className="panel action-panel anchor-panel">
          ...
          <div className="detail-grid">
            ...
            <div>
              ...
              <strong>{props.formatters.formatApprovalStatus(selectedProcedure.status)}</strong>
            </div>
            <div>
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: "actionType", header: "操作", render: (row) => props.formatters.formatApprovalStatus(row.actionType) },
          ...
          <div className="button-row">
            <button type="button" onClick={() => void props.actions.onWorkProcedureDecision(selectedProcedure.id, "approve")}>
              この届出を承認
            </button>
            <button type="button" className="secondary" onClick={() => void props.actions.onWorkProcedureDecision(selectedProcedure.id, "return")}>
              この届出を差戻し
            </button>
          </div>
        </section>
      ...
      {activePanel === "leave-grant-adjust" ? (
        <section id="leave-grant-adjust" className="panel action-panel anchor-panel">
          ...
          <div className="button-row">
            <button type="button" onClick={() => void props.actions.onBulkWorkProcedureDecision("approve")}>
              表示中を一括承認
            </button>
            <button type="button" className="secondary" onClick={() => void props.actions.onBulkWorkProcedureDecision("return")}>
              表示中を一括差戻し
            </button>
          </div>
          <label>
